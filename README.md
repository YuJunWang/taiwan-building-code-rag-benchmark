...
## 🚀 V2 架構升級成果 (V2 Enhancements)

根據第一階段的 Benchmark 實測結果，我們已經在最新的 V2 資料庫中完成了以下重大架構升級：

### 1. Hybrid RAG：導入「父子文件檢索 (Parent-Child Retriever)」
不改變 Vector DB 的切塊大小，但在 MetaData 中綁定「完整條文 (parent_text)」。現在檢索到小切塊時，系統會自動還原整條法規，徹底解決了原本 Top-K 檢索導致脈絡斷裂的痛點。

### 2. Graph RAG：導入「圖譜與原文綁定 (Graph-Document Binding)」
在建立圖譜實體節點 (Node) 的同時，將原始的「法規原文 (raw_text)」直接寫入節點屬性中。當 Graph Traversal 擴展時，LLM 不僅能看到節點間的關聯邊 (Edge)，還能一併讀取最詳盡的法條原文。

### 3. OKF LLM Wiki：建立「主題式 MOC (Thematic Indexes)」
除了原有的章節階層外，系統會根據全域 YAML Tags，自動彙整出**跨章節的主題式 MOC**（例如 `_theme_防火區劃.md`）。Agent 現在處理散佈在各章節的複雜問題時，可以一鍵調閱所有相關連結！

> 💡 **想看看 V2 升級後的震撼效果嗎？**
> 我們使用升級後的系統對 15 個刁鑽問題進行了重新測試，請見：
> 👉 **[V2 架構 Benchmark 詳細測試報告](docs/benchmark_v2_report.md)**
